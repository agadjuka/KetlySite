...
const CLOSE_BTN_CLASS =
  "shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-lg border border-white/10 bg-white/5 hover:bg-white/10 transition-colors";
...
interface VyonTryOnInstructionsMobileFlowProps {
  onDone: () => void;
  onClose: () => void;
}
...
export function VyonTryOnInstructionsMobileFlow({
  onDone,
  onClose,
}: VyonTryOnInstructionsMobileFlowProps) {
    ...
    return (
      <div className="h-full flex flex-col">
        ...
        <div className="flex-none flex items-start justify-between gap-3 px-5 pt-4 pb-3 border-b border-white/[0.06]">
          ...
          <button type="button" onClick={onClose} aria-label="Close" className={CLOSE_BTN_CLASS}>
        ...
        {/* Фото — занимают всё оставшееся место */}
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* Карусель — занимает всё оставшееся место */}
      <div className="flex-1 min-h-0 overflow-hidden py-3">
        ...
        >
          ...
          {/* Peek-хвост для визуального намёка на прокрутку */}
