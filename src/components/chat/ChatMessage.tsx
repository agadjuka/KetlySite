...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  return (
    <div className={`flex gap-3 items-center ${isUser ? 'justify-end' : 'justify-start'}`}>
      {!isUser && (
        <div className={`flex-shrink-0 w-8 h-8 flex items-center justify-center transition-colors duration-700 ease-in-out ${isDemoMode ? 'ring-2 ring-yellow-400/40 rounded-full' : ''}`}>
      ...
      <div
        className={`max-w-[85%] sm:max-w-[80%] rounded-2xl px-3 sm:px-4 py-2.5 sm:py-3 transition-colors duration-700 ease-in-out ${isUser
          ...
          : isDemoMode
            ? 'bg-yellow-900/50 text-yellow-100 border border-yellow-400/40'
